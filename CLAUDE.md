...
- **CV Studio — raw LaTeX đã có**: tab *LaTeX* sửa thẳng `.tex`, lưu ở `cv_versions.meta.tex_override` (JSONB, không migration). JSON **vẫn** là nguồn sự thật cho CV *nói gì*; override chỉ đè cách nó *serialize*. Đánh đổi **một chiều** — không gì parse `.tex` về JSON — nên khi override còn hiệu lực thì field **không tới được PDF**, và UI phải nói ra (banner + dòng dưới tiêu đề). Ba luật: lưu document **không** xoá override (xoá có `DELETE` riêng; `PUT {"files":{}}` là **422**, không phải delete ngầm); **rollback mang override về theo** ("khôi phục v7" = bản PDF v7 đã tạo); path validate trước khi thành path thật (tương đối, không traversal, **không backslash** — `PurePosixPath` đọc `a\b.tex` là một segment vô hại còn Windows thì không, chỉ `.tex`). Editor là textarea + gutter, **không Monaco**.
- **Khung preview PDF**: chiều cao tính từ trang, không ghim px. Viewer chèn ~4px mỗi bên + ~3px trên và fit trang vào phần *bên trong* padding đó, nên tỉ lệ A4 trần luôn **thừa** → hiện nền tối của viewer. Ước lượng **hụt 2px có chủ ý** rồi clip. Overlay full-screen phải **portal ra `<body>`**: nằm trong subtree `sticky` của rail thì header dính vẽ đè, nút Close bấm không được. Blob URL chỉ revoke **sau khi** bản mới về — revoke trong cleanup của effect nghĩa là huỷ đúng lúc compile *bắt đầu*.
- **CV Studio còn thiếu**: HTML live preview, theme gallery. Diff giữa 2 version bất kỳ **đã có** (Phase 16) — dùng chung `tailor/diff.py` với CV Review, chỉ khác bộ chữ (`VERSION_LABELS` vs `TAILOR_LABELS`). Thêm loại so sánh mới thì thêm một `DiffLabels`, **đừng fork hàm diff**: hai bộ luật song song sẽ lệch nhau lúc nào không biết.
