...
- **CV Studio — raw LaTeX đã có** (chi tiết ở `PHASES.md`): override lưu ở `cv_versions.meta.tex_override`. JSON vẫn là nguồn sự thật cho CV *nói gì*; override chỉ đè cách nó *serialize*, và đánh đổi **một chiều** (không gì parse `.tex` về JSON) nên khi nó còn hiệu lực thì field **không tới được PDF** — UI phải nói ra. Lưu document **không** xoá override (`DELETE` mới xoá; `PUT {"files":{}}` là 422); **rollback mang override về theo**; path validate trước khi thành path thật (**chặn cả backslash** — `PurePosixPath` đọc `a\b.tex` là một segment vô hại, Windows thì không).
- **Khung preview PDF**: chiều cao tính từ trang chứ không ghim px — viewer chèn padding rồi fit trang vào *bên trong* nó, nên tỉ lệ A4 trần luôn thừa ra một mảng nền tối. Overlay full-screen phải **portal ra `<body>`** (trong subtree `sticky` thì header dính vẽ đè, bấm không được). Blob URL chỉ revoke **sau khi** bản mới về.
