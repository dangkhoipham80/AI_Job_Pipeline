...
- ⬜ Phase 7–8 — Slack (kênh phụ) → orchestration & polish (PLAN.md §9). Nợ kỹ thuật: CV Studio (HTML live preview, theme gallery, raw LaTeX Monaco, diff giữa 2 version); tailor/apply chạy đồng bộ (Phase 8 đẩy vào job queue); cover letter mới ở dạng text (chưa render `.tex`/PDF như SKILL.md §2 mô tả); `prefill_with_browser` chưa verify được vì chưa cài Playwright; Gmail OAuth chưa làm — dùng `method: smtp` (Gmail app password chạy được).
